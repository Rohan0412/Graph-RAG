...
COSMOS_DATABASE = os.getenv("AZURE_COSMOS_DATABASE_NAME", "graphrag_db")
COSMOS_CONTAINER = os.getenv(
    "AZURE_COSMOS_CONTAINER_NAME", "graphrag_graph"
)  # using container to store nodes+edges
...
def upsert_entity(container, entity: Dict[str, Any]):
    # store entity as document: id, name, type, pk
    doc = {
...
def upsert_relationship(container, rel: Dict[str, Any]):
    # store relationship as document referencing source/target by id
    doc = {
...
def build_graph_from_chunks(
    openai_client: AzureOpenAI, container, chunks: List[Dict[str, Any]]
):
    """For each chunk, extract entities & relations and persist to Cosmos container."""
    for chunk in tqdm(chunks, desc="Extract entities"):
        parsed = llm_extract_entities_and_relations(openai_client, chunk["text"])
        entities = parsed.get("entities", [])
        relationships = parsed.get("relationships", [])
        # upsert entities
        for e in entities:
            # ensure id; the LLM may provide names; if no id, create one from name
            if "id" not in e:
                e["id"] = str(
                    uuid.uuid5(uuid.NAMESPACE_OID, e.get("name", "") + str(time.time()))
                )
            upsert_entity(container, e)
        # upsert relationships
        for r in relationships:
            # map names to entity ids if necessary (assumes LLM provided source_id/target_id)
            if "id" not in r:
                r["id"] = str(uuid.uuid4())
            upsert_relationship(container, r)
    print("Graph build completed and saved to Cosmos.")
...
def run_indexing_pipeline():
    ...
    # build_graph_from_chunks(openai_client, container, all_chunks)
